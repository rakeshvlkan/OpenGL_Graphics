...
ModelClass::ModelClass(bool gamma):gammaCorrection(gamma)
{	
}
...
void ModelClass::Draw(ShaderManager shader)
{
	for (unsigned int i = 0; i < meshes.size(); i++)
		meshes[i].Draw(shader);
}

std::vector<TextureClass> ModelClass::getLoadedTexture()
{
	return textures_loaded;
}

std::vector<MeshClass> ModelClass::getMeshes()
{
	return meshes;
}

void ModelClass::LoadModel(std::string const & path)
{
...
MeshClass ModelClass::processMesh(aiMesh * mesh, const aiScene * scene)
{
	// data to fill
...
std::vector<TextureClass> ModelClass::loadMaterialTextures(aiMaterial * mat, aiTextureType type, std::string typeName)
{
	std::vector<TextureClass> textures;
	for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
	{
		...
		{   // if texture hasn't been loaded already, load it
			...
			texture.SetTextureID(TextureFromFile(str.C_Str(), this->directory));
